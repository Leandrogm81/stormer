...
# --- Função para Obter Dados de Ações (Versão Robusta) ---
...
def calculate_rsi(data, n=2):
    ...
    if "Close" not in data.columns or len(data) < n + 1:
        return data # Retorna os dados originais se não for possível calcular
        ...
        if in_position:
            ...
            if exit_reason:
                ...
                trades.append({
                    ...
                    "Exit Date": current_date if exit_reason == "Alvo" else data.index[i+1] if "Tempo" in exit_reason else current_date,
...
param_oversold = st.sidebar.slider("Nível Sobrevenda IFR(2)", 1, 50, 10)
...
if st.sidebar.button("Iniciar Backtest"):
    ...
    if not tickers:
        ...
        for ticker in tickers:
            ...
            if stock_data is not None:
                ...
                trades_list, _ = run_ifr2_backtest(
                    ticker, stock_data.copy(), param_oversold, param_target_days, param_time_stop, param_shares
                )
                if trades_list:
        ...
        if all_trades:
            ...
            col3.metric("Lucro/Prejuízo Total", f"R$ {total_pnl:,.2f}")
        else:
